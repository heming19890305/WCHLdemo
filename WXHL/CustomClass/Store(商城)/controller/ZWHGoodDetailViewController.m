...
@interface ZWHGoodDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIWebViewDelegate,DismissDelegate,ShareWayClick>
...
@implementation ZWHGoodDetailViewController
...
-(void)getEvaluData{
    MJWeakSelf
    [HttpHandler getCommentList:@{@"id":_goodId,@"pageNo":@"1",@"pageSize":@"1",@"type":_state} Success:^(id obj) {
        if (ReturnValue == 200) {
...
-(void)bottomClicked:(UIButton *)sender{
    ...
    switch (sender.tag) {
        ...
        case 120:
        {
            ...
            if ([_model.collectionFlag isEqualToString:@"1"]) {
                ShowSuccessWithStatus(@"您已收藏该宝贝");
                return;
            }
            if (_model.extendLists.count != 0) {
                [self showNorSelectView];
...
-(void)getcollect{
    ...
    [self getCollectDict];
    MJWeakSelf
    [HttpHandler getaddCollection:_collectDict Success:^(id obj) {
        if (ReturnValue == 200) {
...
-(void)getoPay{
    ...
    if (self.selectView.norDict) {
        ...
        if (self.selectView.selectNumber > [mo.stock integerValue]) {
...
-(void)getDictinfo{
    ...
    if (self.selectView.norDict) {
        ...
        ZWHNorGoodModel *mo = [ZWHNorGoodModel mj_objectWithKeyValues:self.selectView.norDict];
        [_carDict setValue:mo.goodsId forKey:@"goodid"];
        ...
        if ([_state integerValue] == 0) {
            [_carDict setValue:_goodId forKey:@"goodid"];
        }
        ...
        [_carDict setValue:_model.nowPrice forKey:@"price"];
    }
}

#pragma mark - 获得收藏
-(void)getCollectDict{
    _collectDict = [NSMutableDictionary dictionary];
    if (self.selectView.norDict) {
        ZWHNorGoodModel *mo = [ZWHNorGoodModel mj_objectWithKeyValues:self.selectView.norDict];
        if ([_state integerValue] == 0) {
            [_collectDict setValue:mo.goodsId forKey:@"goodid"];
        }
        [_collectDict setValue:mo.id forKey:@"attrid"];
        NSString *str = [[UserManager token] copy];
        [_collectDict setValue:str forKey:@"v_weichao"];
        [_collectDict setValue:_state forKey:@"type"];
    }else{
        if ([_state integerValue] == 0) {
            [_collectDict setValue:_goodId forKey:@"goodid"];
        }
        //[_carDict setValue:_model.attrId forKey:@"attrid"];
        [_collectDict setValue:[UserManager token] forKey:@"v_weichao"];
        [_collectDict setValue:_state forKey:@"type"];
    }
}
