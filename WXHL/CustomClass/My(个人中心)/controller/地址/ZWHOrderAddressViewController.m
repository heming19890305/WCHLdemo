//
//  ZWHOrderAddressViewController.m
//  WXHL
//
...
@interface ZWHOrderAddressViewController ()<UITableViewDelegate,UITableViewDataSource>{
    NSArray *_titleArray;
}
...
@implementation ZWHOrderAddressViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        _swith.on = YES;;
        _swith.userInteractionEnabled = NO;
    ...
    NotAllowUser
        [HttpHandler getaddConsignees:_dict Success:^(id obj) {
            if (ReturnValue == 200) {
                ...
                if (_clicked) {
                    _clicked([ZWHAddressModel mj_objectWithKeyValues:_dict]);
                }
                ONEPOP
            }else{
                ShowInfoWithStatus(ErrorMessage);
