                ...
                isMyOrNo:(BOOL)myOr
{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        ...
        [self.footButton setTitle:footTitle forState:UIControlStateNormal];
        ...
        self.footButton.titleLabel.font = [UIFont systemFontOfSize:17];
        [self.footButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 15, 0, 0)];
        ...
        if (myOr) {
            if (style == GYZSheetStyleDefault) {
                [self upDefaultStyeWithItems:itemTitles title:title isMyOrNo:YES];
                [self pushDefaultStyeSheetView];
            }
        }else{
            if (style == GYZSheetStyleDefault) {
                [self upDefaultStyeWithItems:itemTitles title:title isMyOrNo:NO];
                [self pushDefaultStyeSheetView];
            }
        }
    }
...
///初始化默认样式
- (void)upDefaultStyeWithItems:(NSArray *)itemTitles title:(NSString *)title isMyOrNo:(BOOL)my
{
    //半透明背景按钮
    ...
    if (title.length > 0) {
        if (!my) {
            self.titleView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kMW, kCellH)];
...
//            self.titleLabel.font = [UIFont systemFontOfSize:16];
            ...
            [self.titleLabel setTitleEdgeInsets:UIEdgeInsetsMake(0, 19, 0, 20)];
            [self.titleView addSubview:self.titleLabel];
            [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(self.titleView).with.offset(0);
                make.right.equalTo(self.titleView.mas_right).with.offset(0);
...
//            self.titleLabel.font = [UIFont systemFontOfSize:16];
            ...
            [self.titleView addSubview:self.titleLabel];
            [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(self.titleView).with.offset(SCREEN_WIDTH / 3 );
                make.right.equalTo(self.titleView.mas_right).with.offset(-(SCREEN_WIDTH / 3));
                make.top.equalTo(self.titleView);
                make.height.equalTo(self.titleView);
            }];
            isTitle = YES;
    ...
    [self.footButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 19, 0, 20)];
